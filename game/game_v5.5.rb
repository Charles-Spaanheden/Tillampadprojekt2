...
def animate_sword_slash
    ...
    slash_start_time = Time.now
  
    # Keep track of the sword slash object
    slash_object = @sword_slash
  
    update do
      elapsed_time = Time.now - slash_start_time
  
      if elapsed_time >= @slash_duration / 1000.0
        @sword_slash.color = 'black'
        update_sword_slash
  
        # Remove only the sword slash object
        slash_object.remove
  
        # Stop the update block after executing once
        clear
      end
    end
end

def check_sword_slash_collision
  return unless @player_alive && @sword_slash_active

  # Iterate through enemies and check for collision with sword slash
  @enemies.each do |enemy_data|
    next unless enemy_data[:alive]
    enemy = enemy_data[:enemy]

    if @sword_slash.x < enemy.x + enemy.width &&
       @sword_slash.x + @sword_slash.width > enemy.x &&
       @sword_slash.y < enemy.y + enemy.height &&
       @sword_slash.y + @sword_slash.height > enemy.y
...
def player_attack
  return if @sword_slash_active

  @sword_slash_active = true
  animate_sword_slash
